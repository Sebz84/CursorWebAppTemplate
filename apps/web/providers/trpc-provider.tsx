            ...
            async headers() {
              ...
              if (getToken) {
                try {
                  ...
                  console.warn(
                    '[tRPC] Failed to fetch Clerk token with template "integration_fallback", falling back to default template.',
                    error
                  );
                  token = await getToken().catch((fallbackError) => {
                    console.error('[tRPC] Failed to fetch Clerk token without template.', fallbackError);
                    return null;
                  });
                }
              }
